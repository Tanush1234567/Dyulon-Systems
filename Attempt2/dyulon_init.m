...
p.motor_type = [ 1, 1, 2,  2];   % 1=hover prop (front, puller)  2=cruise prop (rear, pusher)
p.has_tilt   = [ 1, 1, 1,  1];   % REV 4: all four motors tilt independently
                                  % (no fixed-mount motors -- ailerons are the
                                  %  only non-motor control surface on the airframe)
p.thrust_dir = [ 1, 1, -1, -1];  % REV 4: +1 = puller (thrust +x at beta=0)
                                  %        -1 = pusher (thrust -x at beta=0)

%% =========================================================================
%  6. TILT ACTUATORS  (REV 4 -- direct-drive gear+motor tilt mechanism,
%     NOT a servo. All 4 motors tilt independently. No lateral tilt.)
%
%  BETA CONVENTION (REV 4 -- changed from Rev 3, do not mix):
%    beta = 0 deg    : thrust horizontal (front = +x fwd, rear = -x aft)
%    beta = +90 deg  : thrust straight up (hover)
%    beta > 90 deg   : tilted PAST vertical, thrust gains a component
%                       toward the *opposite* horizontal direction
%                       (front motor at 100 deg points slightly aft-and-up)
%    This is the opposite sign convention from Rev 3 (which had beta=0 at
%    vertical). Every consumer of beta must use REV 4 convention below.
%
%  ROLE SPLIT (per user hardware config + control authority decision):
%    Front (pullers, hover-optimised 22" prop): VERTICAL-heavy.
%      Nominal/hover operating point near beta=90 deg. Primary hover lift
%      and primary ROLL authority (differential Omega_FL vs Omega_FR,
%      most roll-efficient near vertical). Tilts down toward 0 deg for
%      cruise, where a puller is aerodynamically clean.
%    Rear (pushers, cruise-optimised 18" prop): HORIZONTAL-heavy.
%      Nominal/cruise operating point near beta=0 deg. Primary cruise
%      thrust and PITCH-TRIM authority via the long moment arm (x_rear),
%      assisting hover lift and providing pitch differential against front.
%      Excursion toward vertical is for hover assist only, not nominal.
%    YAW: differential beta (tilt angle) on either pair, in addition to
%      differential reaction torque. This is new authority Rev 3 didn't
%      have (Rev 3 only had fixed-mount rear motors contributing reaction
%      torque). Still the weakest axis -- no vertical fin.
%% =========================================================================
p.beta_min_front = deg2rad(-15);   % rad  front hard stop (generous default, TBD)
p.beta_max_front = deg2rad(100);   % rad  front hard stop (generous default, TBD)
p.beta_min_rear  = deg2rad(-15);   % rad  rear  hard stop (generous default, TBD)
p.beta_max_rear  = deg2rad(100);   % rad  rear  hard stop (generous default, TBD)

p.beta_nom_front = deg2rad(90);    % rad  front nominal/hover operating tilt
p.beta_nom_rear  = deg2rad(0);     % rad  rear  nominal/cruise operating tilt

% Direct-drive gear+motor tilt actuator dynamics.
% NOT a servo -- no manufacturer datasheet yet. Modelled as 2nd-order
% (same structural form as Rev 3's servo model) with a corner frequency
% and rate limit that are PLACEHOLDERS pending bench characterisation of
% the actual gearmotor. Flagged VERY HIGH sensitivity below -- replace
% before any hardware-in-the-loop test.
p.tilt_omega_n   = 25;    % rad/s  PLACEHOLDER -- bench test the real gearmotor
p.tilt_zeta      = 1.2;   % -      PLACEHOLDER -- overdamped assumption only
p.tilt_rate_lim  = 6.0;   % rad/s  PLACEHOLDER -- gearmotors are typically
                           %        slower than dedicated servos; verify

% Explicit-Euler stability bound for dyulon_actuator_dynamics.m.
% Rule of thumb: dt < 0.1 / omega_n for a stable, reasonably accurate
% forward-Euler integration of the 2nd-order tilt dynamics.
p.dt_actuator_max = 0.1 / p.tilt_omega_n;   % s
...
%% =========================================================================
%  15. STATE VECTOR DEFINITION  (REV 4 -- 23 elements, all 4 motors tilt)
...
%
%  CHANGED FROM REV 3: rear motors now have tilt states (beta_tilt_RL,
%  beta_tilt_RR) since they are no longer fixed-mount. Vector grew from
%  21 to 23 elements. Anything hardcoded to the old 21-element layout
%  (Simulink Selector blocks with fixed indices, MATLAB Function blocks
%  unpacking by position) MUST be re-checked against the new indices below.
%% =========================================================================
...
ic.vel_E          = 0;
ic.vel_D          = 0;
ic.beta_tilt_FL   = p.beta_nom_front;   % rad  REV 4: start at nominal hover tilt
ic.beta_tilt_FR   = p.beta_nom_front;
ic.beta_tilt_RL   = p.beta_nom_front;   % rad  REV 4: rear starts vertical too for
ic.beta_tilt_RR   = p.beta_nom_front;   %      a hover-trim initial condition
...
%  [14] vel_E       m/s
%  [15] vel_D       m/s
%  [16] beta_tilt_FL rad   REV 4: front-left tilt actuator state
%  [17] beta_tilt_FR rad   REV 4: front-right tilt actuator state
%  [18] beta_tilt_RL rad   REV 4 NEW: rear-left tilt actuator state
%  [19] beta_tilt_RR rad   REV 4 NEW: rear-right tilt actuator state
%  [20] Omega_FL    rad/s
%  [21] Omega_FR    rad/s
%  [22] Omega_RL    rad/s
%  [23] Omega_RR    rad/s
