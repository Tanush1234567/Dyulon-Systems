...
%  Rev 4 - all four motors tilt independently (no fixed-mount rear motors).
%  BETA CONVENTION (REV 4, changed from Rev 3 -- do not mix):
%    beta = 0 rad    : thrust horizontal (front/puller = +x fwd,
%                       rear/pusher = -x aft, via p.thrust_dir)
%    beta = +pi/2    : thrust straight up (hover)
%    beta > pi/2     : tilted past vertical, gains thrust component
...
%    p  - parameter struct from dyulon_init.m (REV 4: p.has_tilt = [1 1 1 1],
%         p.thrust_dir = [1 1 -1 -1] for puller/pusher sign)
...
for i = 1:4
    ...
    % REV 4: all four motors tilt (p.has_tilt is all-ones, kept as a field
    % for hardware-config flexibility, e.g. simulating a locked actuator).
    if p.has_tilt(i)
        beta_i = actuators.beta(i);
    else
        beta_i = p.beta_nom_front;   % fallback if a motor's tilt is disabled
    end

    % Thrust unit vector, REV 4 convention:
    %   beta=0    -> thrust_dir(i)*[1;0;0]   (horizontal, puller fwd / pusher aft)
    %   beta=pi/2 -> [0;0;-1]                (straight up)
    %   beta>pi/2 -> swings toward -thrust_dir(i)*x (past vertical)
    thrust_axis = [p.thrust_dir(i) * cos(beta_i); 0; -sin(beta_i)];
    T_vec = T_i * thrust_axis;

    % Reaction torque along thrust axis
    Q_vec = -p.spin_dir(i) * Q_i * thrust_axis;
    ...
    % Gyroscopic moment
    H_rotor   = p.spin_dir(i) * I_rotor * Omega_i * thrust_axis;
