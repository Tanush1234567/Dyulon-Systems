...
%% ---- 9. Tilt Servo Step Response ----
subplot(3,4,9);
sys_tilt = tf(p.servo_omega_n^2, [1, 2*p.servo_damping*p.servo_omega_n, p.servo_omega_n^2]);
[y_tilt, t_out] = step(sys_tilt, t_sim);
plot(t_out*1000, y_tilt, 'g-','LineWidth',2);
yline(0.63,'k:'); yline(1.02,'r:');
xlabel('Time  [ms]'); ylabel('Normalised Response  [-]');
title(sprintf('Tilt Servo Step Response (\\omega_n = %g, \\zeta = %.1f)', p.servo_omega_n, p.servo_damping));
grid on; ylim([0 1.15]);

%% ---- 10. Transition Blending vs Airspeed ----
subplot(3,4,10);
V_blend = linspace(0, 20, 200);
blend_val = min(1, max(0, (V_blend - p.V_blend_lo)/(p.V_blend_hi - p.V_blend_lo)));
beta_front_deg = rad2deg(p.beta_min * blend_val);   % front tilt angle schedule
yyaxis left;
plot(V_blend, blend_val, 'b-','LineWidth',2);
ylabel('Blend factor  [0=hover, 1=cruise]');
yyaxis right;
plot(V_blend, beta_front_deg, 'r--','LineWidth',2);
ylabel('Front tilt angle  [deg]');
xlabel('Airspeed  [m/s]');
title('Transition Blending Schedule');
